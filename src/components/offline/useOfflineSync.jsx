...
export const useOfflineSync = () => {
  ...
  const syncCollectes = useCallback(async () => {
    if (!navigator.onLine) { setSyncError('Pas de connexion internet'); return { success: false, error: new Error('Pas de connexion internet') }; }
    setIsSyncing(true); setSyncError(null);
    const pending = await offlineDb.getPendingCollectes();
    let syncedCount = 0; const errors = [];
    for (const item of pending) {
      const { localId, serverId, operation, status, retryCount, lastError, createdAt, updatedAt, syncedAt, ...data } = item;
      try {
        let result;
        if (operation === 'update' && serverId) result = await base44.entities.Collecte.update(serverId, data);
        else result = await base44.entities.Collecte.create(data);
    ...
    await updatePendingCount(); setIsSyncing(false);
    if (errors.length) setSyncError(errors.join('\n'));
    return { success: errors.length === 0, syncedCount, errorCount: errors.length, errors, message: `${syncedCount} synchronisée${syncedCount > 1 ? 's' : ''}${errors.length ? `, ${errors.length} erreur(s)` : ''}` };
  }, [updatePendingCount]);
